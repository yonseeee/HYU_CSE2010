...
AVLNode* RotateRight(AVLNode* node){
    ...
    node->left=L->right;//p->left를 L->right로
    L->right=node;//L->right를 p로
    updateHeight(node);
    updateHeight(L);//p와 L의 height update
    return L;//return L
}
...
AVLNode* RotateLeft(AVLNode* node){
    ...
    node->right=R->left;//p->right를 R->left로
    R->left=node;//R->left를 p로
    updateHeight(node);
...
AVLNode* Insert(Element X, AVLNode* T){
    ...
    if(X<T->element)T->left=Insert(X,T->left);//T->element != X면 recursive하게 찾고,
...
AVLNode* Delete(Element X, AVLNode* T){
    ...
    if(X<T->element)T->left=Delete(X,T->left);//Insert와 동일하게 recursive
			...
			T->element=p->element;//그 노드의 값으로 current->value 설정
			T->right=Delete(p->element,T->right);//우측 자식의 가장 좌측 자식 삭제 recursive
		}
		else if(T->left!=NULL){// 자식이 왼쪽 자식 하나일 경우 -> free current -> return 자식
